...
def analyze_membership(config_file):
    ...
    if ("skip_network_cluster_stability" not in config_json["parameters"]) or (json.loads(config_json["parameters"]["skip_network_cluster_stability"].lower()) is False):
        ...
        cluster_pair_wgcna_n_com1, network_cluster_stability1 = network_cluster_stability(gene_to_module_mapping_df, comm_df1, 'louvain_label', expression_meta_df)
        for cluster in comm_df1.louvain_label.unique():
            plot_random_vs_actual_z(gene_to_module_mapping_df, comm_df1, cluster_pair_wgcna_n_com1[cluster], cluster, 'louvain_label', network_cluster_stability1, 'wgcna vs louvain 1', expression_meta_df)
