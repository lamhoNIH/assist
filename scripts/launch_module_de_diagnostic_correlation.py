...
if len(sys.argv) != 2:
    print("python launch_module_de_diagnostic_correlation.py <dataset>")
    exit(2)
...
system(f'docker run --rm -m 10g -e config_file="{data}/{config_path}/{module}.json" -v "{data}":"{data}" assist/{module}:0.1.0')
print(f"{datetime.now()}")