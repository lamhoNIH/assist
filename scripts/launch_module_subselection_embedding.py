...
if len(sys.argv) != 2:
    print("python launch_module_subselection_embedding.py <dataset>")
    exit(2)
...
system(f'docker run --rm -m 24g -e config_file="Data/{output_path}/config.json" -e archive_path="Data/{output_path}" -e run_num="run3" -v "{data}":/assist/Data assist/{module}:0.1.0')
print(f"{datetime.now()}")